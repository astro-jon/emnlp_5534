...
import torch
import numpy as np
from dataset import InputDataset, InputDatasetOption, InputDatasetWithVocab
...
def train(model, fold, train_loader, val_loader, test_loader, log):
    ...
    best_model_path = CFG.save_path + f'{CFG.name}_batch{CFG.batch_size}_dropout{CFG.dropout}_lr{CFG.learning_rate}/'
...
def main():
    ...
    result = []
    for fold in range(5):
        tr_data = pd.read_csv('../data/bigbird_sentence_high/fold_{}/train.csv'.format(fold))
        va_data = pd.read_csv('../data/bigbird_sentence_high/fold_{}/val.csv'.format(fold))
        test_data = pd.read_csv('../data/bigbird_sentence_high/fold_{}/test.csv'.format(fold))

        # tr_data = tr_data[tr_data['level'] == 0].reset_index(drop=True)
        # va_data = va_data[va_data['level'] == 0].reset_index(drop=True)
        # test_data = test_data[test_data['level'] == 0].reset_index(drop=True)
